...
const PublicGrievances = () => {
  ...
  const fetchGrievances = async () => {
    try {
      const response = await fetch(
        `http://${localhost}:3000/api/warden/getpublicgrievance`,
        {
  ...
  const updateGrievanceStatus = async (grievanceId, status) => {
    ...
    try {
      const response = await fetch(
        `http://${localhost}:3000/api/warden/setpublicgrievance/${grievanceId}`,
        {
          method: "POST",
          headers: {
    ...
    <View style={styles.container}>
      ...
      {selectedGrievance && (
        <Modal transparent={true} animationType="slide">
          <TouchableWithoutFeedback onPress={() => setSelectedGrievance(null)}>
