...
const ViewAttendance = () => {
  ...
  const fetchHostlers = async () => {
    ...
    setLoading(true);
    try {
      const formattedDate = selectedDate.toISOString().split("T")[0]; // Convert to "yyyy-mm-dd" format (date only)
      ...
      if (!response.ok) {
        throw new Error(`Failed to fetch hostlers. Status: ${response.status}`);
      }
      ...
      <View style={styles.inputContainer}>
        <View style={styles.dropdownContainer}>
          ...
          >
            ...
            <Picker.Item label="Sarojni Naidu" value="Sarojni Naidu" />
          </Picker>
        </View>
      </View>
